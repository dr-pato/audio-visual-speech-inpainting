        ...
        if config['model'] == 'av-blstm-twosteps':
            ...
            with tf.variable_scope(config['model']):
                if config['model'] == 'a-blstm':
                    ...
                    model = net.StackedBLSTMSSNNModel(sequence_lengths_ph, target_sources_ph, masks_ph, audio_feat_mean_ph,
                                                      audio_feat_std_ph, dropout_rate_ph, config, input='v', video_features=video_features_ph, is_training=False)
            ...
            for n_epoch in range(config['max_n_epochs']):
                ...
                while True:
                    try:
                        ...
                        length_batch, length_lab_batch, target_audio_batch, sample_path_batch, labels_batch, \
                            video_batch, mask_batch = sess.run(next_train_batch)
                        max_seq_len = max(length_batch)
                        
                        loss, loss_fn, lr, _, emb, emb_ext = sess.run(fetches=[model.loss, model.loss_func, model.learning_rate, model.train_op, model.speaker_embedding, model.speaker_embedding_ext],
                                            feed_dict={
                ...
                while True:
                    try:
                        ...
                        length_batch, length_lab_batch, target_audio_batch, sample_path_batch, \
