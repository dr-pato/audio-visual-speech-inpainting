...
def encoder_layer(x, filter_size, num_filters, batch_norm=True):
    stddev = np.sqrt(2 / (filter_size ** 2 * num_filters))
    w = tf.Variable(tf.truncated_normal([filter_size, filter_size, tf.shape(x)[3], num_filters], stddev=stddev), name="w")
    b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")

    conv_2d = tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding='SAME')
    conv_2d = tf.nn.bias_add(conv_2d, b)
    if batch_norm:
        conv_2d = tf.layers.batch_normalization(conv_2d, training=True)
    conv_2d = tf.nn.relu(conv_2d)

    return conv_2d
    
def decoder_layer(x, e_conv, filter_size, num_filters, batch_norm=True):
    stddev = np.sqrt(2 / (filter_size ** 2 * num_filters))
    w = tf.Variable(tf.truncated_normal([filter_size, filter_size, tf.shape(x)[3], num_filters], stddev=stddev), name="w")
    b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")

    up_x = tf.keras.UpSampling((2,2))(x)
    concat_x = tf.concat(3, [e_conv, up_x])
    conv_2d = tf.nn.conv2d(concat_x, w, strides=[2, 2, 1, 1], padding='SAME')
    conv_2d = tf.nn.bias_add(conv_2d, b)
    if batch_norm:
        conv_2d = tf.layers.batch_normalization(conv_2d, training=True)
