...
def serialize_sample_fixed(seq_len, lab_len, target_audio_wav, video_features, mask, labels, embedding, sample_path):
    ...
    example.context.feature['embedding'].float_list.value.extend(embedding)
...
def serialize_sample_var(seq_len, lab_len, target_audio_wav, video_features, mask, labels, embedding, sample_path):
    # The object we return
    example = tf.train.SequenceExample()
...
def create_tfrecords_training(data_path, dest_dir, ph_dict, tfrecord_mode='fixed'):
    ...
    for i, sample_dir in enumerate(sample_dir_list):
        ...
        # Read embedding
        embedding = np.load(os.path.join(sample_dir, 'vgg_embeddings', 'target.npy'))
        ...
        with tf.python_io.TFRecordWriter(tfrecord_file) as writer:
            ...
            if tfrecord_mode == 'fixed':
                serialized_sample = serialize_sample_fixed(seq_len, lab_len, target_audio_wav, video_features, mask, labels, embedding, sample_path)
            elif tfrecord_mode == 'var':
                serialized_sample = serialize_sample_var(seq_len, lab_len, target_audio_wav, video_features, mask, labels, embedding, sample_path)
