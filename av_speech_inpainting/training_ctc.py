    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
            ...
            if config['model_ckp']:
                ...
                training_log_file.write('## CTC-loss coefficient: {:.6f}\n'.format(config['ctc_loss']))
            ...
            print('## CTC-loss coefficient: {:.6f}'.format(config['ctc_loss']))
            ...
            for n_epoch in range(config['max_n_epochs']):
                ...
                while True:
                    try:
                        ...
                        loss, loss_ipt, loss_ctc, per, lr, _ = sess.run(fetches=[model.loss, model.loss_hole, model.ctc_loss, model.per, model.learning_rate, model.train_op],
                                            feed_dict={
                                              sequence_lengths_ph: length_batch,
                                              labels_lengths_ph: length_lab_batch,
                        ...
                        if n_step == 1:
                            ...
                            train_avg_loss = loss
                            train_avg_loss_ipt = loss_ipt
                            train_avg_loss_ctc = loss_ctc
                            ...
                            train_avg_loss_ipt = (train_avg_loss_ipt * prev_nframe_sum + loss_ipt * np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']) / nframe_sum
                            train_avg_loss_ctc = (train_avg_loss_ctc * prev_nframe_sum + loss_ctc * np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']) / nframe_sum
                            train_avg_per = (train_avg_per * prev_nframe_sum + per.mean() * np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']) / nframe_sum
                            
                        # Print loss value fairly often.
                        if n_step % 200 == 0 or n_step == 1:
                            print('Step[{:7d}] Loss[{:3.5f}|{:3.5f}|{:3.5f}] PER[{:.5f}] LR[{:.6f}] Epoch training time[{:.2f}]'. \
                                format(tot_step, train_avg_loss, train_avg_loss_ipt, train_avg_loss_ctc, train_avg_per, lr, time() - epoch_start_time))
                        if n_step % 1000 == 0:
                            save_path = saver.save(sess, os.path.join(checkpoints_dir, 'ckpt'))
                            print('Model checkpoint saved in file %s' % save_path)

                    except tf.errors.OutOfRangeError:
                        print('Completed epoch {:d} at step {:d} --> Training loss: {:3.5f} - {:3.5f} - {:3.5f}; PER: {:3.5f}'. \
                            format(epoch_counter, tot_step, train_avg_loss, train_avg_loss_ipt, train_avg_loss_ctc, train_avg_per))
                ...
                while True:
                    try:
                        ...
                        if n_step == 1:
                            loss, loss_ipt, loss_ctc, per, summaries = sess.run(fetches=[model.loss, model.loss_hole, model.ctc_loss, model.per, model.summaries],
                                                       feed_dict={
                                                        sequence_lengths_ph: length_batch,
                                                        labels_lengths_ph: length_lab_batch,
                                                        target_sources_ph: target_audio_batch,
                            ...
                            loss, loss_ipt, loss_ctc, per = sess.run(fetches=[model.loss, model.loss_hole, model.ctc_loss, model.per],
                                            feed_dict={
                                              sequence_lengths_ph: length_batch,
                                              labels_lengths_ph: length_lab_batch,
                                              target_sources_ph: target_audio_batch,
                        ...
                        if n_step == 1:
                            nframe_sum = np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']
                            val_avg_loss = loss
                            val_avg_loss_ipt = loss_ipt
                            val_avg_loss_ctc = loss_ctc
                            ...
                            val_avg_loss_ipt = (val_avg_loss_ipt * prev_nframe_sum + loss_ipt * np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']) / nframe_sum
                            val_avg_loss_ctc = (val_avg_loss_ctc * prev_nframe_sum + loss_ctc * np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']) / nframe_sum
                            val_avg_per = (val_avg_per * prev_nframe_sum + per.mean() * np.count_nonzero(mask_batch == 0) // config['audio_feat_dim']) / nframe_sum


                        if n_step % 200 == 0 or n_step == 1:
                            print('Step[{:7d}] Loss[{:3.5f}]'.format(n_step, val_avg_loss_ipt))
                    except tf.errors.OutOfRangeError:
                        print('done.')
                        print('Validation loss: {:3.5f}; PER: {:3.5f}. Best loss so far {:2.5f} [Epoch {:d} (step {:d})]'. \
                                format(val_avg_loss_ipt, val_avg_per, best_val_loss, best_val_checkpoint[0], best_val_checkpoint[1]))
                    
                        # Save checkpoint if the current model is the best on validation set
                        if best_val_checkpoint == (0, 0) or val_avg_loss_ipt < best_val_loss:
                            checkpoint_path = os.path.join(checkpoints_dir, 'sinet')
                            save_path = saver.save(sess, checkpoint_path)
                            print('Model saved in file %s' % save_path)
                            best_val_checkpoint = (epoch_counter, tot_step)
                            best_val_loss = val_avg_loss_ipt
                            cneg_epochs = 0
                        else:
                            cneg_epochs += 1
                ...
                tb_summary.value.add(tag='Training loss inpainting', simple_value=train_avg_loss_ipt)
                tb_summary.value.add(tag='Training loss CTC', simple_value=train_avg_loss_ctc)
                tb_summary.value.add(tag='Training loss PER', simple_value=train_avg_per)
                tb_summary.value.add(tag='Validation loss', simple_value=val_avg_loss)
                tb_summary.value.add(tag='Validation loss inpainting', simple_value=val_avg_loss_ipt)
                tb_summary.value.add(tag='Validation loss CTC', simple_value=val_avg_loss_ctc)
                tb_summary.value.add(tag='Validation loss PER', simple_value=val_avg_per)
                ...
                training_log_file.write('{:d}\t{:.6f}\t{:.6f}|{:.6f}|{:.6f}\t{:.6f}\t{:.6f}|{:.6f}|{:.6f}\t{:.6f}\t[{:.2f}]\n' \
                   .format(epoch_counter, lr, train_avg_loss, train_avg_loss_ipt, train_avg_loss_ctc, train_avg_per,
                           val_avg_loss, val_avg_loss_ipt, val_avg_loss_ctc, val_avg_per, epoch_duration))
...
if __name__ == '__main__':
    config_file = 'C:\\Users\\Giovanni Morrone\\Documents\\Dottorato di Ricerca\\Internship AAU\\code\\scripts\\Inpainting\\configurations\\blstm_ssnn-ctc_grid.config'
