...
def speech_inpainting_eval(test_audio_dir, enhanced_file, out_file, masked_eval=True, pesq_path='pesq', pesq_mode='nb', n_fft=512, window_size=25, step_size=10):
    ...
    per_masked = []
    labels_list = []
    decoded_enhanced_list = []
    decoded_masked_list = []

    # Compute speech enhancement eval metrics
    i = 0
    for ex_dir in sample_dirs:
        ...
        with open('transcription.lbl') as f:
            labels = f.read()
            labels_list.append(labels)
            labels = labels.split(',')
        with open(os.path.join('transcriptions', enhanced_file + '.lbl')) as f:
            decoded_enhanced = f.read()
            decoded_enhanced_list.append(decoded_enhanced)
            decoded_enhanced = decoded_enhanced.split(',')
        ...
        # PER
        per_enhanced.append(edit_eval(labels, decoded_enhanced) / len(labels))

        if masked_eval:
            _, masked = wavfile.read('masked.wav')
            with open('masked.lbl') as f:
                decoded_masked = f.read()
                decoded_masked_list.append(decoded_masked)
                decoded_masked = decoded_masked.split(',')
            ...
            # PER
            per_masked.append(edit_eval(labels, decoded_masked) / len(labels))
            
            print('[Masked|Enhanced] L1: {:5f}|{:5f} - PESQ: {:.5f}|{:5f} - STOI: {:.5f}|{:5f} - PER: {:.5f}|{:5f}'. \
                format(l1_masked[i], l1_enhanced[i], pesq_masked[i], pesq_enhanced[i], stoi_masked[i], stoi_enhanced[i], per_masked[i], per_enhanced[i]))
        else:
            print('[Enhanced] L1: {:.5f} - PESQ: {:.5f} - STOI: {:.5f} - PER: {:.5f}'.format(l1_enhanced[i], pesq_enhanced[i], stoi_enhanced[i], per_enhanced[i]))
        i += 1
    ...
    print('Enhanced PER: {:.5f} ({:.5f})'.format(np.mean(per_enhanced), np.std(per_enhanced)))
    if masked_eval:
        print('')
        ...
        print('Masked PER: {:.5f} ({:.5f})'.format(np.mean(per_masked), np.std(per_masked)))
        print('')
        ...
        per_r = np.array(per_masked) - np.array(per_enhanced)
        ...
        print('PER reduction: {:.5f} ({:.5f})'.format(np.mean(per_r), np.std(per_r)))
    ...
    with open(results_file, 'w') as f:
        ...
        if masked_eval:
            wr.writerow(['SAMPLE', 'L1_MASK', 'L1_ENH', 'PESQ_MASK', 'PESQ_ENH', 'STOI_MASK', 'STOI_ENH', 'PER_MASK', 'PER_ENH', 'L1r', 'PESQi', 'STOIi', 'PERr', 'LAB', 'DEC_ENH', 'DEC_MASK']) # header
            data = list(zip(sample_names, l1_masked, l1_enhanced, pesq_masked, pesq_enhanced, stoi_masked, stoi_enhanced, per_masked, per_enhanced, l1_r, pesq_i, stoi_i, per_r, labels_list, decoded_enhanced_list, decoded_masked_list))
            data.sort(key=lambda x: x[0]) # sort rows by filename
            wr.writerows(data) # results
        else:
            wr.writerow(['SAMPLE', 'L1_ENH', 'PESQ_ENH', 'STOI_ENH', 'PER_ENH', 'LAB', 'DEC_ENH']) # header
            data = list(zip(sample_names, l1_enhanced, pesq_enhanced, stoi_enhanced, per_enhanced, labels_list, decoded_enhanced_list))
            data.sort(key=lambda x: x[0]) # sort rows by filename
            wr.writerows(data) # results
