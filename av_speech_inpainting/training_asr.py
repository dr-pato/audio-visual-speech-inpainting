...
def train(config_file):
    ...
    data_path_train = os.path.join(data_path, 'test-set-lbl')
    data_path_val = os.path.join(data_path, 'test-set-lbl')
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('placeholder'):
            ...
            audio_feat_mean_ph = tf.placeholder(tf.float32, shape=[len(audio_feat_mean)], name='audio_features_mean')
            audio_feat_std_ph = tf.placeholder(tf.float32, shape=[len(audio_feat_mean)], name='audio_features_std')
            dropout_rate_ph = tf.placeholder(tf.float32, name='dropout_rate')
            
        # Graph building and definition.
        print('Building ASR training model:')
        with tf.variable_scope('asr/' + config['model']):
            if config['model'] == 'a-blstm':
                model = net.StackedBLSTMModel(sequence_lengths_ph, labels_lengths_ph, target_sources_ph, masks_ph, labels_ph,
                                              audio_feat_mean_ph, audio_feat_std_ph, dropout_rate_ph, config, input='a', is_training=False)
            elif config['model'] == 'v-blstm':
                model = net.StackedBLSTMModel(sequence_lengths_ph, labels_lengths_ph, target_sources_ph, masks_ph, labels_ph,
                                              audio_feat_mean_ph, audio_feat_std_ph, dropout_rate_ph, config, input='v', video_features=video_features_ph)
            elif config['model'] == 'av-blstm':
                model = net.StackedBLSTMModel(sequence_lengths_ph, labels_lengths_ph, target_sources_ph, masks_ph, labels_ph,
                                              audio_feat_mean_ph, audio_feat_std_ph, dropout_rate_ph, config, input='av', video_features=video_features_ph)
            else:
                print('Model selection must be "a-blstm", "v-blstm", "av-blstm". Closing...')
                sys.exit(1)
            model.build_graph(var_scope='asr/' + config['model'])
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
            ...
            if config['model_ckp']:
                ...
                training_log_file.write('\nEpoch\tLR\tTraining loss\tTraining PER\tValidation loss\tValidation PER\t[TIME]\n')
            ...
            best_val_per = -1.0 # Save best validation Frame Error Rate
            ...
            for n_epoch in range(config['max_n_epochs']):
                ...
                while True:
                    try:
                        ...
                        lengths_batch, lengths_lab_batch, target_audio_batch, sample_path_batch, labels_batch, \
                            video_batch, mask_batch = sess.run(next_train_batch)
                        max_seq_len = max(lengths_batch)
                        
                        loss, loss_fn, per, lr, _ = sess.run(fetches=[model.loss, model.ctc_loss, model.per, model.learning_rate, model.train_op],
                                            feed_dict={
                                              sequence_lengths_ph: lengths_batch,
                                              labels_lengths_ph: lengths_lab_batch,
                        ...
                        if n_step == 1:
                            lengths_sum = lengths_lab_batch.sum()
                            train_avg_loss = loss
                            train_avg_loss_fn = loss_fn
                            train_avg_per = per.mean()
                        else:
                            prev_lengths_sum = lengths_sum
                            batch_lab_len_sum = lengths_lab_batch.sum()
                            lengths_sum += batch_lab_len_sum
                            train_avg_loss = (train_avg_loss * prev_lengths_sum + loss * batch_lab_len_sum) / lengths_sum
                            train_avg_loss_fn = (train_avg_loss_fn * prev_lengths_sum + loss_fn * batch_lab_len_sum) / lengths_sum
                            train_avg_per = (train_avg_per * prev_lengths_sum + per.mean() * batch_lab_len_sum) / lengths_sum
                            
                        # Print loss
                        if n_step % 200 == 0 or n_step == 1:
                            print('Step[{:7d}] Loss[{:3.5f}|{:3.5f}] PER[{:.5f}] LR[{:.6f}] Epoch training time[{:.2f}]'. \
                                format(tot_step, train_avg_loss, train_avg_loss_fn, train_avg_per, lr, time() - epoch_start_time))
                        if n_step % 1000 == 0:
                            save_path = saver.save(sess, os.path.join(checkpoints_dir, 'ckpt'))
                            print('Model checkpoint saved in file %s' % save_path)

                    except tf.errors.OutOfRangeError:
                        print('Completed epoch {:d} at step {:d} --> Training loss: {:3.5f} - {:3.5f}, PER: {:.5f}'. \
                            format(epoch_counter, tot_step, train_avg_loss, train_avg_loss_fn, train_avg_per))
                ...
                while True:
                    try:
                        ...
                        lengths_batch, lengths_lab_batch, target_audio_batch, sample_path_batch, \
                            labels_batch, video_batch, mask_batch = sess.run(next_val_batch)
                        max_seq_len = max(lengths_batch)

                        loss, per, dec = sess.run(fetches=[model.ctc_loss, model.per, model.decoding],
                                             feed_dict={
                                              sequence_lengths_ph: lengths_batch,
                                              labels_lengths_ph: lengths_lab_batch,
                                              target_sources_ph: target_audio_batch,
                        ...
                        if n_step == 1:
                            lengths_sum = lengths_lab_batch.sum()
                            val_avg_loss = loss
                            val_avg_per = per.mean()
                        else:
                            prev_lengths_sum = lengths_sum
                            batch_lab_len_sum = lengths_lab_batch.sum()
                            lengths_sum += batch_lab_len_sum
                            val_avg_loss = (val_avg_loss * prev_lengths_sum + loss * batch_lab_len_sum) / lengths_sum
                            val_avg_per = (val_avg_per * prev_lengths_sum + per.mean() * batch_lab_len_sum) / lengths_sum

                        if n_step % 200 == 0 or n_step == 1:
                            print('Step[{:7d}] Loss[{:3.5f}] PER[{:5f}]'.format(n_step, val_avg_loss, val_avg_per))
                    except tf.errors.OutOfRangeError:
                        print('done.')
                        print('Validation loss: {:3.5f}, PER: {:.5f}. Best PER so far {:5f} [Epoch {:d} (step {:d})]'. \
                                format(val_avg_loss, val_avg_per, best_val_per, best_val_checkpoint[0], best_val_checkpoint[1]))
                    
                        # Save checkpoint if the current model is the best on validation set
                        if best_val_checkpoint == (0, 0) or val_avg_per < best_val_per:
                            checkpoint_path = os.path.join(checkpoints_dir, 'asrnet')
                            save_path = saver.save(sess, checkpoint_path)
                            print('Model saved in file %s' % save_path)
                            best_val_checkpoint = (epoch_counter, tot_step)
                            best_val_per = val_avg_per
                            cneg_epochs = 0
                        else:
                            cneg_epochs += 1
                ...
                tb_summary.value.add(tag='Training PER', simple_value=train_avg_per)
                tb_summary.value.add(tag='Validation loss', simple_value=val_avg_loss)
                tb_summary.value.add(tag='Validation PER', simple_value=val_avg_per)
                ...
                training_log_file.write('{:d}\t{:.6f}\t{:.6f}|{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t[{:.2f}]\n' \
                   .format(epoch_counter, lr, train_avg_loss, train_avg_loss_fn, train_avg_per,
                           val_avg_loss, val_avg_per, epoch_duration))
        ...
        print('Best validation checkpoint: {:d} ({:d}) - PER: {:.5f}'.format(best_val_checkpoint[0], best_val_checkpoint[1], best_val_per))


if __name__ == '__main__':
    config_file = 'C:\\Users\\Giovanni Morrone\\Documents\\Dottorato di Ricerca\\Internship AAU\\code\\scripts\\Inpainting\\configurations\\asr_blstm_grid.config'
