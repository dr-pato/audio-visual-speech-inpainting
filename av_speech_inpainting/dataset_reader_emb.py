...
class DataManager:
    ...
    def __init__(self, num_audio_samples=48000, audio_feat_size=257, video_feat_size=136, embedding_size=512, buffer_size=1000, mode='fixed'):
    ...
    def read_data_format_fixed(self, sample):
        context_parsed, sequence_parsed = \
                    tf.parse_single_sequence_example(sample,
                                                     context_features={
                                                      ...
                                                      'embedding': tf.FixedLenFeature([self.embedding_size], dtype=tf.float32),
        ...
        return tf.to_int32(context_parsed['sequence_length']), tf.to_int32(context_parsed['labels_length']), \
            tf.to_int32(context_parsed['target_audio_wav']), context_parsed['embedding'], context_parsed['sample_path'], sequence_parsed['labels'], \
    ...
    def read_data_format_var(self, sample):
        ...
        return tf.to_int32(context_parsed['sequence_length']), tf.to_int32(context_parsed['labels_length']), \
            context_parsed['embedding'], sequence_parsed['target_audio_wav'], sequence_parsed['sample_path'], \
            sequence_parsed['labels'], sequence_parsed['video_features'], sequence_parsed['mask']


if __name__ == '__main__':
    data_path = 'C:\\Users\\Public\\aau_data\\GRID\\tfrecords\\test_si_dataset\\test-set-sample'
    ...
    data_manager = DataManager(num_audio_samples, audio_feat_size, video_feat_size, embedding_size, 1000, tfrecords_mode)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
        ...
        while True:
            # Fetch batch.
            lengths, lab_lengths, target_audio_wavs, embeddings, sample_paths, labels, video_feat, masks = sess.run(next_batch)
